...
from .tools.LayerUtils.LayerConvertorTool import LayerConvertorTool
...
class bpla_plugin_flightsDialog(QtWidgets.QDialog, FORM_CLASS):
    ...
    def initActiveLayersComboBox(self):
        lg = LayerGetter()
        self.layersList = lg.getActiveLayers(iface.mapCanvas())
        GuiElemIFace().setComboBox(self.comboBox, self.layersList)
    ...
    def doResult(self):
        ...
        self.layer = lg.layer
        self.driverName = lg.driverName
        self.layername = lg.layername
        self.layerpath = lg.layerpath
        self.csvFileAttrs = lg.csvFileAttrs

        self.getFilepath()

        if self.layer and self.filepath is not None:
            if self.driverName == "Delimited text file":
                LayerConvertorTool().csvToMemory(self.textEdit, self.layerpath, self.csvFileAttrs)
            elif self.driverName == "ESRI Shapefile":
                LayerConvertorTool().layerToMemory(self.textEdit, self.layer, self.driverName, self.layerpath)
            # self.removeZeroPointsFromMemory()
            # self.mainAzimutCalc()
            LayerConvertorTool().saveTempLayerToFile(self.textEdit, self.filename, self.filepath, QgsProject.instance())
        else:
            GuiElemIFace().setTextStyle(self.textEdit, 'red', 'bold', 'Введите данные в форму!\n')
